...
# =================================================
# 1️⃣3️⃣ SAVE DOCX REPORT (QUESTION-WISE TABLE FORMAT)
# =================================================
doc = Document()
doc.add_heading("RAG Evaluation Report", level=1)

# -------------------------------
# GLOBAL GENERATION METRICS
# -------------------------------
doc.add_heading("Global Generation Metrics", level=2)

gen_table = doc.add_table(rows=1, cols=2)
gen_table.style = "Table Grid"
gen_table.rows[0].cells[0].text = "Metric"
gen_table.rows[0].cells[1].text = "Value"

def add_row(tbl, k, v):
    r = tbl.add_row().cells
    r[0].text = str(k)
    r[1].text = f"{v:.4f}" if isinstance(v, float) else str(v)

add_row(gen_table, "BLEU", compute_bleu(answers, references))
r1, rL = compute_rouge(answers, references)
add_row(gen_table, "ROUGE-1", r1)
add_row(gen_table, "ROUGE-L", rL)
add_row(gen_table, "METEOR (lite)", compute_meteor_lite(answers, references))
add_row(gen_table, "BERTScore F1", compute_bertscore(answers, references))
...
for i, q in enumerate(questions):
    ...
    # IR METRICS TABLE
    # -------------------------------
    doc.add_heading("IR Metrics", level=3)
    ir_tbl = doc.add_table(rows=1, cols=2)
    ir_tbl.style = "Table Grid"
    ir_tbl.rows[0].cells[0].text = "Metric"
    ir_tbl.rows[0].cells[1].text = "Value"
    ...
    # RAGAS METRICS TABLE
    # -------------------------------
    doc.add_heading("RAGAS Metrics", level=3)
    ragas_tbl = doc.add_table(rows=1, cols=2)
    ragas_tbl.style = "Table Grid"
    ragas_tbl.rows[0].cells[0].text = "Metric"
    ragas_tbl.rows[0].cells[1].text = "Value"

    for k, v in ragas_results[i].items():
        if isinstance(v, float):
            add_row(ragas_tbl, k, v)

    # -------------------------------
    # CUSTOM RAG QUALITY METRICS
    # -------------------------------
    doc.add_heading("Custom RAG Quality Metrics", level=3)
    custom_tbl = doc.add_table(rows=1, cols=2)
    custom_tbl.style = "Table Grid"
    custom_tbl.rows[0].cells[0].text = "Metric"
    custom_tbl.rows[0].cells[1].text = "Value"

    rel, ctx = answer_relevance_context_utilization(
        [answers[i]], [references[i]], [retrieved_docs_per_q[i]]
    )
    coh, read = coherence_readability([answers[i]])

    add_row(custom_tbl, "Answer Relevance", rel)
    add_row(custom_tbl, "Context Utilization", ctx)
    add_row(custom_tbl, "Groundedness", groundedness([answers[i]], [retrieved_docs_per_q[i]]))
    add_row(custom_tbl, "Hallucination Rate", hallucination_rate([answers[i]], [retrieved_docs_per_q[i]]))
    add_row(custom_tbl, "Coherence (words/sentence)", coh)
    add_row(custom_tbl, "Readability (Flesch)", read)
    add_row(custom_tbl, "Query Relevancy", query_relevancy([answers[i]], [q]))
...
print("📄 Output file: rag_evaluation_report.docx")
